...
class File {
	...
	static void add_invalid(const std::string &hash) { file_append(invalid_filename, hash); }
	...
	load_run(std::function<std::shared_ptr<Run>(const std::vector<std::string> &)> factory) {
		using namespace std;
	...
	static std::set<std::string> load_blacklist() {
		...
		for (auto &filename : {blacklist_filename, incompatible_filename, invalid_filename,
